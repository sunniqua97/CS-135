...
This program will also implement the function bool timeOverlap(TimeSlot ts1, TimeSlot ts2); which will determine whether
two TimeSlots are overlapping. If they overlap, it will return true, false otherwise.
...
bool timeOverlap(TimeSlot ts1, TimeSlot ts2){
	...
	int timediff = minutesUntil(ts1.startTime, ts2.startTime);
	
	if (timediff < 0){
		
		timediff = timediff * -1;
		return ts2.movie.duration >= timediff;
		...
		return ts1.movie.duration >= timediff;
